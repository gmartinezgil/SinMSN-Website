 ...
 * @author Roger Chen
 */
public class IncomingVER extends MsnIncomingMessage implements Serializable {
    ...
    public MsnProtocol[] getSupportedProtocol() {
    	final List<MsnProtocol> protocols = new ArrayList<MsnProtocol>();
        for (int i = 0; i < getParamCount(); i++) {
        	final MsnProtocol protocol = MsnProtocol.parseStr(getParam(i));
            if (protocol != null) {
            	protocols.add(protocol);
            }
        }
        return protocols.toArray(new MsnProtocol[protocols.size()]);
    }
